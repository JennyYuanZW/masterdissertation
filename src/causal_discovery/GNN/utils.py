...
def gauss_sample_z(logits, zsize):
    ...
    for j in range(logits.size(0)):
        x[j, 0, :] = (
            U[j, :] * torch.exp(logits[j, 0, zsize : 2 * zsize])
            + logits[j, 0, 0:zsize]
        )
    return x
...
def list_files(directory, extension):
    return (
        f for f in os.listdir(directory) if f.endswith("_graph" + extension)
    )
...
def load_data_discrete(args, batch_size=1000, suffix="", debug=False):
    ...
    if args.data_type == "synthetic":
        ...
        if args.data_filename.endswith(".pkl"):
            with open(
                os.path.join(args.data_dir, args.data_filename), "rb"
            ) as handle:
...
def load_data(args, batch_size=1000, suffix="", debug=False):
    ...
    df = pd.read_csv(
        os.path.join(args.data_dir, args.data_filename), index_col=0
    )
    data = df.values
    ...
    for i in range(num_buckets):
        if i < num_buckets - 1:
            idx = np.where(
                np.all(
                    np.vstack(
                        (dist > thresholds[i], dist <= thresholds[i + 1])
                    ),
                    0,
                )
            )[0]
...
def compute_local_BiCScore(np_data, target, parents):
    ...
    for data_ind in range(sample_size):
        parent_combination = tuple(
            np_data[data_ind, parents].reshape(1, -1)[0]
        )
